...
river_params_df = pd.read_excel("Chirchiq_gidro_uzel_uchun_mothly_discharge,_kengik,_eni,_chuqurligi.xlsx")
...
decay_rates = {
    'COD': 5.0e-5, 'BOD': 1.0e-4, 'NH4': 5.0e-5,
    'NO2': 5.0e-5, 'NO3': 0.0, 'P': 0.0,
    'TSS': 0.0, 'Temp': 0.0
}
...
future_month_name = st.sidebar.selectbox("Oyni tanlang", list(month_num_map.keys()), index=4)
param = st.sidebar.selectbox("Parametrni tanlang", parameters, index=1)

if st.sidebar.button("Bashorat qilish"):

    future_month_num = month_num_map[future_month_name]

    data = df_down.dropna(subset=['Year', 'Month_num', param]).copy()

    X_train = data[['Year', 'Month_num']]
    y_train = data[param]

    model = RandomForestRegressor(n_estimators=300, random_state=42)
    model.fit(X_train, y_train)

    future_input = pd.DataFrame([[future_year, future_month_num]], columns=['Year', 'Month_num'])
    predicted_value = model.predict(future_input)[0]

    river_row = river_params_df[river_params_df['Month_num'] == future_month_num]
    ...
    if river_row.empty:
        ...
        C = predicted_value * np.exp(-decay_rate * X_grid / U) * np.exp(
        ...
        ax.set_title(f"{future_year}-yil | {future_month_name} | {param} downstream bashorat dispersion modeli")
        ax.set_xlabel("Masofa (km)")
        ax.set_ylabel("Daryo kengligi (m)")
        ax.scatter(0, 0, color="red", marker="*", label="Boshlang‘ich nuqta")
        ax.legend()
        ax.grid(True)
